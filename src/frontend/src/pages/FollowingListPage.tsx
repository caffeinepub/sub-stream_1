...
function formatFollowDate(ts: number | null): string {
  ...
  if (days === 0) return "Following since today";
  if (days === 1) return "Following since yesterday";
  if (days < 30) return `Following for ${days} days`;
  const months = Math.floor(days / 30);
  if (months < 12)
    return `Following for ${months} month${months > 1 ? "s" : ""}`;
  const years = Math.floor(months / 12);
  return `Following for ${years} year${years > 1 ? "s" : ""}`;
}
...
export function FollowingListPage({
  ...
  // Fetch following principals
  const { data: followingPrincipals = [], isLoading: loadingPrincipals } =
    useQuery({
      queryKey: ["following", userId],
      queryFn: async () => {
        if (!actor || !targetPrincipal) return [];
        return actor.getFollowing(targetPrincipal);
      },
      enabled: !!actor && !!targetPrincipal,
    });

  // Fetch profiles for each following
  const { data: followingProfiles = [], isLoading: loadingProfiles } = useQuery(
    {
      queryKey: ["followingProfiles", userId, followingPrincipals.length],
      queryFn: async () => {
        if (!actor || followingPrincipals.length === 0) return [];
        const results = await Promise.all(
          followingPrincipals.map(async (p) => {
            try {
      ...
      enabled: !!actor && followingPrincipals.length > 0,
    },
  );

  // Track which principals the current user follows
  const { data: myFollowingSet = new Set<string>() } = useQuery({
  ...
  return (
    ...
    >
      ...
      >
        <button
          type="button"
          data-ocid="following-list.back_button"
          onClick={onBack}
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        {isLoading && (
          <div
            data-ocid="following-list.loading_state"
        ...
        {!isLoading && followingProfiles.length === 0 && (
          <motion.div
            data-ocid="following-list.empty_state"
          ...
          >
            ...
            <p className="text-white/35 text-sm leading-relaxed max-w-xs">
              Creators that {displayName} follows will appear here.
            </p>
          </motion.div>
        )}
        ...
        {!isLoading && followingProfiles.length > 0 && (
          <div className="px-4 pt-4 space-y-2">
            {followingProfiles.map(({ principal: pStr, profile }, i) => {
              const name = profile
                ? getDisplayName(profile.name) || "USER"
                : "USER";
              const uname = profile ? getUsername(profile.name) : "";
              const avatar = profile?.avatarUrl ?? "";
              const followTs = getFollowTimestampStatic(userId, pStr);
              const isMe = pStr === myPrincipal;
              const iAmFollowing = myFollowingSet.has(pStr);

              return (
                <motion.div
                  key={pStr}
                  data-ocid={`following-list.item.${i + 1}`}
                ...
                >
                  ...
                  {/* Follow/Unfollow button — only for authenticated and not own profile */}
                  {isAuthenticated && !isMe && (
                    <button
                      type="button"
                      data-ocid={`following-list.follow_button.${i + 1}`}
                      onClick={() =>
